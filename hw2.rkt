...
(module+ test
  ...
  (test (parse '{max 2 1})
        (maxC (numC 2) (numC 1 )))
  (test (parse '{max (+ 2 3) 8})
        (maxC (plusC (numC 2) (numC 3)) (numC 8)))
  (test/exn (parse '{{+ 1 2}})
            "invalid input")
  ...
  (test (parse-fundef '{define {add x y} {+ x y}})
        (fdC 'add (list 'x 'y) (plusC (idC 'x) (idC 'y))))
...
(define (interp [a : ExprC] [env : Env] [fds : (listof FunDefC)]) : number
  (type-case ExprC a
    ...
    [appC (s arg) (local [(define fd (get-fundef s fds))]
                    (if (eq? (length (fdC-arg fd)) (length arg))
                        (interp (fdC-body fd)
                             (map2 bind (fdC-arg fd)
                                   (local [(define (f a) (interp a env fds))]
                                     (map f arg)))
                            fds)
    ...
    [letC (n rhs body)
          (interp body
                  ...
                  fds)]
    [maxC (lhs rhs)
         (local [(define l (interp lhs env fds))
                 (define r (interp rhs env fds))]
           (if (> l r)
               l r))]
    [unletC (s body)
            (interp body
...
;;delete-bind ---------------------------------
(define (delete-bind [s : symbol] [env : Env] ): Env
  (cond
    [(empty? env) empty]
    [(cons? env)
     (if (eq? s (bind-name (first env)))
         (rest env)
         (cons (first env)
               (delete-bind s (rest env) )))]))
